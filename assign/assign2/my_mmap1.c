...
int my_open(struct inode *inode, struct file *filp)
{
    ...
    memcpy(info->data + 32, file_dentry(filp)->d_name.name,
           strlen(file_dentry(filp)->d_name.name));
...
static int __init mmapexample_module_init(void)
{
    ...
    printk(KERN_INFO "MyMMAP Module added.\n");
    return 0;
}
...
static void __exit mmapexample_module_exit(void)
{
    ...
    cdev_del(&c_dev);
    unregister_chrdev_region(dev, MINOR_CNT);
    printk(KERN_INFO "MyMMAP Module removed.\n");
